...
function bubbleChart() {
  ...
  var yearCenters = {
    ...
    GPE: { x: width/2, y: height / 2 },
    ORG: { x: 2*width/3, y: height / 2 }
  };
  ...
  var fillColor = d3.scale.ordinal()
    .domain(['PERSON', 'GPE', 'ORG'])
    .range(['#e32636', '#5d8aa8', '#4CA54C']);
...
function tick(e) {
    bubbles.each(gravity(.2 * e.alpha))
        .each(collide(.5))
        .attr("cx", function (d) {
        return d.x;
    })
...
var format = d3.format(",d");
  ...
  var chart = function chart(selector, rawData) {
    ...
    .each(function(d, i) {
      var nm = d.name;
      var arr = nm.split(" "),arrlength = (arr.length > 7) ? 8 : arr.length;
      console.log(arr);
      d3.select(this).attr('y',"-" + (arrlength/2) + "em");
      //if text is over 7 words then ellipse the 8th
      for(var n = 0; n < arrlength; n++) {
        var tsp = d3.select(this).append('tspan').attr("x", d.x).attr("y", d.y).attr("data-classname", nm);
        if(n === 7) {
          tsp.text("...");
        } else {
          tsp.text(arr[n]);
...
// Load the data.
d3.csv('data/ents.csv', display);

// setup the buttons.
setupButtons();


function visit_link(d) {
    window.open("https://www.google.gr/search?q="+d.name); //urlLink;
}
